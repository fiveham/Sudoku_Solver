 ...
 *
 */
public class SledgeBrute extends AbstractTechnique {
	 ...
	 */
	public SledgeBrute(Sudoku puzzle) {
		super(puzzle);
	...
	private static void populateDistinctRules(SledgeBrute sh){
		...
		sh.distinctSourcesBySledgehammerSize.putAll(mapRulesBySize(sh.distinctRules.stream(), SledgeBrute::sledgehammerSizeIfSource));
	}
	...
	public static Map<Integer,List<Rule>> mapRulesBySize(Stream<Rule> ruleStream, Function<? super Rule, ? extends Integer> sledgehammerSize){
		return ruleStream.collect(Collectors.toMap(
				sledgehammerSize, 
				SledgeBrute::singletonList, 
				SledgeBrute::mergeCollections));
	}
