...
public class Wrap<W> implements WrapVertex<W,Wrap<W>>{
	 ...
	 * <p>This method connects the nodes in {@code O(n^2)} time where 
	 * {@code n = unwrapped.size()} by testing all possible pairs of nodes.</p>
	 * <p>The generated edges are bidirectional.</p>
	 * @param unwrapped raw nodes
	 * @param edgeDetector outputs true if two raw nodes from {@code unwrapped} share 
	 * an edge, false otherwise
	...
	/**
	 * <p>A utility method that wraps each element of {@code unwrapped} in a {@code T} 
	 * specified by {@code wrapper} and gives each resulting {@code T} neighbors from 
	 * among the resulting {@code T}s. Two resulting {@code T}s are neighbors if 
	 * {@code edgeDetector} {@link BiPredicate#test(Object,Object) says so}.</p>
	 * <p>This method connects the nodes in {@code O(n^2)} time where 
	 * {@code n = unwrapped.size()} by testing all possible pairs of nodes.</p>
	 * <p>The generated edges are bidirectional.</p>
	 * 
	 * @param unwrapped raw nodes
	 * @param edgeDetector outputs true if and only if two raw nodes from 
	 * {@code unwrapped} share an edge, false otherwise
	...
	/**
	 * <p>A utility method that connects the nodes in {@code wrapped} by brute force, 
	 * trying every pair of them and testing the pair using {@code edgeDetector}.</p>
	 * <p>This method operates in {@code O(n^2)} time, where 
	 * {@code n = wrapped.size()}.</p>
	 * @param wrapped the nodes of a graph, wrapped in some type of WrapVertex
	 * @param edgeDetector bidirectionally links two nodes from {@code wrapped} if and 
	 * only if their raw nodes test true
	 */
	...
	/**
	 * <p>A utility method that extracts a graph's nodes from its {@code edges} and 
	 * wraps those nodes in {@code Wrap}s.</p>
	 * @param edges a {@link Collection} of edges of the graph of whose interconnected 
	 * nodes a list is returned
	 * @return a list of the nodes of the graph whose edges are specified 
	 * in {@code edges}
	 * @throws IllegalArgumentException if any of the {@code edges} has a size 
	 * other than {@value #VERTICES_PER_EDGE}
	 */
	...
	/**
	 * <p>A utility method that extracts a graph's nodes from its {@code edges} and 
	 * wraps those nodes with {@code WrapVertex}s via {@code wrapper}.</p>
	 * @param edges a {@link Collection} of edges of the graph of whose interconnected 
	 * nodes a list is returned
	 * @param wrapper specifies the type of {@code WrapVertex} that wraps raw node 
	 * elements of edges from {@code edges}
	 * @return a list of the nodes of the graph whose edges are specified 
	 * in {@code edges}
	 * @throws IllegalArgumentException if any of the specified {@code edges} has a 
	 * size not equal to {@value #VERTICES_PER_EDGE}
	 */
	 ...
	 * {@link Vertex#neighbors() neighbors} of each other such that all the edges 
	 * specified by {@code edges} are accounted for.</p>
	 * <p>This method operates in {@code O(n)} time where 
	 * {@code n = edges.size()}.</p>
	 * @param edges the edges of the graph whose nodes are contained in 
	 * {@code wrappedNodes}
	 * @param wrappedNodes the wrapped nodes of the graph whose edges are contained 
	 * in {@code edges}
	 * @throws IllegalArgumentException if any of the {@code edges} has a size 
	 * not equal to {@value #VERTICES_PER_EDGE}
	 */
	...
	/**
	 * <p>A utility method that wraps the raw nodes of a graph whose edges are 
	 * explicitly specified as {@code edges} and whose nodes are implicitly 
	 * specified via the application of {@code nodeSource} to the elements of 
	 * {@code edges}. The extracted raw nodes are wrapped via {@code wrapper}.</p>
	 * @param edges the edges of the graph in a form that {@code nodeSource} can 
	 * translate into a size-{@value #VERTICES_PER_EDGE} Collection of raw nodes
	 * @param nodeSource translates a raw edge to a size-{@value #VERTICES_PER_EDGE} 
	 * Collection of raw nodes
	 * @param wrapper wraps the raw nodes that {@code nodeSource} extracts from 
	 * the elements of {@code edges}
	 * @return a List of wrapped, connected nodes of the graph whose {@code edges} 
	 * were provided
	 * @throws IllegalArgumentException if any of the Collections produced by 
	 * an application of {@code nodeSource} has a size other than 
